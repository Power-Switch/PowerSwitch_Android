...
import static eu.power_switch.persistence.preferences.SmartphonePreferencesHandler.KEY_AUTO_COLLAPSE_ROOMS;
...
class RoomHandler {
    ...
    protected Room get(@NonNull SQLiteDatabase database, Long id) throws Exception {
        ...
        if (cursor.moveToFirst()) {
            room = dbToRoom(database, cursor);
            boolean collapse = smartphonePreferencesHandler.getValue(KEY_AUTO_COLLAPSE_ROOMS);
    ...
    public LinkedList<Room> getByApartment(@NonNull SQLiteDatabase database, Long apartmentId) throws Exception {
        ...
        boolean autoCollapseRooms = smartphonePreferencesHandler.getValue(KEY_AUTO_COLLAPSE_ROOMS);
