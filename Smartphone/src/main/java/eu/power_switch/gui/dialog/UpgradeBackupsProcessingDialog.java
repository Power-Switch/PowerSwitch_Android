    ...
    protected void onStartProcessing() throws Exception {
        ...
        processingTask = new AsyncTask<Void, Object, AsyncTaskResult<Void>>() {
            ...
            protected AsyncTaskResult<Void> doInBackground(Void... voids) {
                try {
                    String backupPath = smartphonePreferencesHandler.getValue(SmartphonePreferencesHandler.BACKUP_PATH);
                    ...
                    if (backupDir.exists()) {
                        ...
                        for (int i = 0; i < listFiles.length; i++) {
                            ...
                            File target    = new File(smartphonePreferencesHandler.getValue(SmartphonePreferencesHandler.BACKUP_PATH) + File.separator + oldBackup.getName() + BackupHandler.BACKUP_FILE_SUFFIX);
                            if (target.exists()) {
                                target.delete();
                            }
